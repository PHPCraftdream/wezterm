...
pub struct LocalDomain {
    ...
    /// fallback ONLY while `wsl_domains_cached()` returns `None`. As of the
    /// current implementation, the ordering in `config::WslDomain::try_default_domains()`
    /// (cache publish happens before domain registration) means this fallback
    /// is not actually reached by real auto-discovery in production today. It
    /// exists to keep the "cache might be `None`" case handled defensively rather
    /// than as a documented guarantee that some caller can currently exercise.
    wsl_fallback: Option<WslDomainFallback>,
}

/// Snapshot of critical WSL domain fields stored at construction time.
/// Used ONLY as a defensive fallback for the "cache might be `None`" case;
/// never substitutes for a live cached lookup. As of the current implementation,
/// the ordering in `config::WslDomain::try_default_domains()` (cache publish
/// happens before domain registration) means this fallback is not actually reached
/// by real auto-discovery in production today.
#[derive(Clone, Debug)]
...
impl LocalDomain {
    ...
    fn resolve_wsl_domain(&self) -> Option<WslDomain> {
        ...
        //   below locks this in).
        // - the list isn't known *yet* (in theory this could happen if cache
        //   publish and domain registration were reordered, but in practice the
        //   current implementation always publishes the cache before registering
        //   auto-discovered domains): we still know from `is_wsl_domain` that this
