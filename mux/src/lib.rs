...
impl std::future::Future for DomainDiscoveryWaitFuture {
    ...
    fn poll(mut self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context) -> Poll<Self::Output> {
        ...
        // Log after releasing `state`'s lock above, following this file's
        // existing convention of not logging while holding
        // `domain_discovery`'s lock. Gated on `newly_registered` so this
...
impl Mux {
    ...
    ///
    /// Deliberately does *not* try to apply a `pending_default`: the
    /// `InProgress` entry that carried this run's copy is long gone, and the
    /// next config reload applies it from the now-registered domain anyway.
    fn adopt_orphaned_discovery(&self, cancel_epoch_at_start: u64, domains: &[Arc<dyn Domain>]) {
        ...
        *state = DomainDiscoveryState::Done {
            ...
            not_found_pending_default: None,
        };
    ...
    ///
    /// `accepts_as_default` is invoked from two places, both while holding
    /// `domain_discovery`'s lock *and* `default_domain`'s write lock, and
    /// both wrapping the call in `catch_unwind` so a panic in it can't
    ...
    ///
    /// In both cases it must **not** call back into `Mux`'s
        ...
        let mut stuck_wakers: Vec<Waker> = vec![];
        {
            ...
            } = &mut *state
            {
                if started_at.elapsed() < DOMAIN_DISCOVERY_STUCK_THRESHOLD {
                    ...
                    *in_flight_accepts_as_default = Arc::new(accepts_as_default);
                    return;
                }
            ...
            *state = DomainDiscoveryState::InProgress {
                ...
                wakers: HashMap::new(),
                next_waiter_id: 0,
                accepts_as_default: Arc::new(accepts_as_default),
...
mod domain_discovery_tests {
    ...
    /// Covers the race condition raised in review against the original
    /// background-thread fix: spawning directly into a domain by name
    ...
    fn resolve_spawn_tab_domain_waits_for_in_flight_discovery() {
        ...
        // releaser thread doesn't open the gate until after we've had
        // a chance to actually start waiting, proving ordering rather
        // than racing past a bug that returns immediately.
    ...
    /// Regression test for a reload race raised in review: a config reload
    ...
    /// Regression test for a reload race raised in review: switching to an
    ...
    /// Regression test for a bug found in review: `DomainDiscoveryWaitFuture`
    ...
    fn cancel_domain_discovery_in_flight_does_not_prevent_a_later_real_discovery() {
        ...
        gate_tx
            .send(())
            .expect("discovery thread should still be waiting on the gate");
        // Wait for the stale thread to finish and drop its finish guard.
        // Poll for Done state with a generous window to avoid flakiness.
    ...
    /// Regression test proving the panic-safety fix from review: a
    ...
    fn run_late_success_after_replacement_failed(
        ...
        // Run B supersedes the stuck A and fails outright.
        mux.begin_domain_discovery(|| Err(anyhow!("simulated wsl.exe failure")), None);
        mux.await_domain_discovery(DOMAIN_DISCOVERY_TIMEOUT);
        assert!(
            matches!(
                &*mux.domain_discovery.lock(),
    ...
    fn late_success_after_replacement_failed_is_adopted() {
        let mux = run_late_success_after_replacement_failed(None);
    ...
    fn late_success_after_cancellation_is_not_adopted() {
        let mux = run_late_success_after_replacement_failed(Some(Box::new(|mux: &Arc<Mux>| {
    ...
    fn stale_finish_guard_does_not_finish_a_newer_discovery_run() {
        ...
        // Wait for the stale thread to finish before checking state,
        // using a poll loop with a generous window.
        let deadline = Instant::now() + Duration::from_millis(500);
        while Instant::now() < deadline {
            let state = mux.domain_discovery.lock();
            if !matches!(&*state, DomainDiscoveryState::InProgress { .. }) {
